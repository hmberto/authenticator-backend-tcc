...
public class LogoutTest {
	...
	public void runInvalidSessionTokenFormat() throws Exception {
		...
		String body = new JSONObject().put("email", email).put("session", session).put("killAll", false).toString();
	...
	public void runUnregisteredUser() throws Exception {
		...
		runTest(body, "Email Token could not be updated");
	}
	...
	private void runTest(String body, String errorMessage) throws Exception {
		...
		Mockito.when(req.getRequestURI()).thenReturn("/logout");
		Mockito.when(req.getPathInfo()).thenReturn("/logout");
		...
		LogoutService servlet = new LogoutService();
